...
class RevocationCacheEntry:
    ...
    def from_dict(cls, data: dict[str, Any]) -> RevocationCacheEntry:
        ...
        return cls(
            ...
            revocation_timestamp=datetime.fromisoformat(data["revocation_timestamp"])
            if data.get("revocation_timestamp")
            else None,
...
class RevocationCacheAdapter:
    ...
    def __init__(self, cache_manager: Any, default_ttl: int = 3600):
        ...
        Args:
            cache_manager: Async cache implementation exposing get/set/delete
    ...
    async def get(
        ...
        try:
            ...
            logger.warning(
                f"Failed to get revocation cache entry for {fingerprint}: {e}"
            )
            return None
    ...
    async def set(
        ...
        try:
            ...
            logger.debug(
                f"Cached revocation result for {fingerprint}: is_revoked={is_revoked}, ttl={ttl}s"
            )
    ...
    async def delete(
        ...
        try:
            ...
            logger.warning(
                f"Failed to delete revocation cache entry for {fingerprint}: {e}"
            )
            return False
