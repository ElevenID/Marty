"""Register the retained Digital Identity API with a FastAPI application."""
...
class DigitalIdentityPlugin:
    """
    Compatibility application adapter for the Digital Identity API.
    ...
    def register_routes(self, app: FastAPI) -> None:
        ...
        if not self.enabled:
            logger.info(
                "Digital Identity plugin disabled - skipping route registration"
            )
            return
    ...
    def _configure_workflow_integration(self, container: Any) -> None:
        ...
        try:
            if hasattr(container, "resolve"):
                ...
                if workflow_engine:
                    container.register(
                        "digital_identity_workflow_engine", workflow_engine
                    )
    ...
    async def startup(self) -> None:
        ...
        event_publisher = create_event_publisher(
            use_memory=True
        )  # Will be replaced in configure_services
            ...
            try:
                ...
                adapters["ICAO"] = IcaoTrustProfile(
                    trust_store_path=Path(
                        icao_config.get("trust_store_path", "data/csca")
                    ),
                    master_list_sources=icao_config.get("master_list_sources", []),
                    pkd_urls=icao_config.get("pkd_urls", []),
                )
        ...
        if trust_config.get("aamva", {}).get("enabled", True):
            try:
                ...
                adapters["AAMVA"] = AamvaTrustProfile(
                    iaca_directory=Path(
                        aamva_config.get("iaca_directory", "data/iaca")
                    ),
                    vical_url=aamva_config.get("vical_url"),
                    dts_url=aamva_config.get("dts_url"),
                )
...
def register_plugin(
    app: FastAPI, config: dict[str, Any] | None = None
