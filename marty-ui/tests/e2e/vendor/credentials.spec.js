...
test.describe('Vendor Credential Configuration', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    // Wait for login to complete and navigation to vendor dashboard
  ...
  test('vendor can add a new credential type', async ({ page }) => {
    ...
    if (await addButton.isVisible().catch(() => false)) {
      ...
      // Should see a dialog/form for adding credential type
      await page.waitForTimeout(500);
      
      // Look for the credential type selection or form
      const formElements = [
