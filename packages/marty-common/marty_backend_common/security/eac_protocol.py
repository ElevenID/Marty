...
# Use Rust-backed crypto from crypto_bridge
...
class EACChipAuthentication:
    ...
    def _perform_ecdh(self, peer_public_key_bytes: bytes) -> bytes:
        ...
        # Reconstruct peer public key from uncompressed point (x || y)
        # The chip sends raw coordinates; prepend 0x04 if needed
        if len(peer_public_key_bytes) in (64, 96):
            # Raw x||y for P-256 (32+32) or P-384 (48+48) — add uncompressed prefix
            key_bytes = b"\x04" + peer_public_key_bytes
        elif len(peer_public_key_bytes) in (65, 97) and peer_public_key_bytes[0] == 0x04:
            key_bytes = peer_public_key_bytes
        else:
            msg = f"Invalid peer public key length: {len(peer_public_key_bytes)}"
            raise ChipAuthenticationError(msg)

        peer_public_key = ec.EllipticCurvePublicKey.from_encoded_point(
            private_key.curve, key_bytes
        )
    ...
    def _perform_rsa_key_agreement(self, peer_public_key_bytes: bytes) -> bytes:
        """Perform RSA-based key agreement using RSA-KEM"""
        private_key = self.ephemeral_key_pair[1]
        if not isinstance(private_key, rsa.RSAPrivateKey):
            msg = "RSA private key required"
            raise ChipAuthenticationError(msg)

        # The chip encrypts a random secret with our public key (RSA-KEM).
        ...
        try:
            ...
            msg = f"RSA key agreement decryption failed: {e}"
            raise ChipAuthenticationError(msg) from e
...
class EACSecureMessaging:
    ...
    def encrypt_apdu(self, apdu_data: bytes) -> bytes:
        ...
        try:
            ...
            # Encrypt data using AES-256-CBC (Rust)
            iv = secrets.token_bytes(16)
            encrypted_data = bytes(aes_256_cbc_encrypt(
                self.secure_channel.encryption_key, iv, apdu_data,
            ))
    ...
    def decrypt_apdu(self, encrypted_apdu: bytes) -> bytes:
        ...
        try:
            ...
            # Decrypt data using AES-256-CBC (Rust)
            apdu_data = bytes(aes_256_cbc_decrypt(
                self.secure_channel.encryption_key, iv, encrypted_data,
            ))
    ...
    def _calculate_mac(self, data: bytes) -> bytes:
        """Calculate HMAC-SHA256 for secure messaging (Rust)."""
        if not self.secure_channel.mac_key:
            msg = "MAC key not available"
            raise EACError(msg)

        return bytes(hmac_sha256(self.secure_channel.mac_key, data))
