...
# Use Rust crypto_bridge for certificate and hash operations
...
from marty_backend_common.crypto_bridge import sha256 as _sha256

# Fallback if Rust sha256 not available
if _sha256 is None:
    import hashlib as _hashlib
    _sha256 = lambda data: _hashlib.sha256(data).digest()
...
class SdJwtVerifier:
    ...
    def _hash_disclosure(disclosure_bytes: bytes) -> bytes:
        return bytes(_sha256(disclosure_bytes))
