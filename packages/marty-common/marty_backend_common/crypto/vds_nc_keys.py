...
class VDSNCKeyMetadata:
    ...
    def to_jwk(self, public_key: EllipticCurvePublicKey) -> dict[str, Any]:
        ...
        x_b64 = b64url_encode(x_bytes)
        y_b64 = b64url_encode(y_bytes)
...
class VDSNCKeyManager:
    ...
    def __init__(
        ...
        credential_key_manager: Any | None = None,
    ) -> None:
        ...
        Args:
            ...
            credential_key_manager: Optional ICredentialKeyManager for KMS-backed
                key generation and signing.  When provided, keys are generated
                inside the KMS boundary rather than in-process.
        """
        self.session = session
        self.rotation_config = rotation_config or RotationConfig()
        self.generator = VDSNCKeyGenerator()
        self._credential_key_manager = credential_key_manager
    ...
    async def create_key(
        ...
        # Generate key pair — route through KMS when available
        if self._credential_key_manager is not None:
            # KMS-backed: key material stays inside KMS boundary
            key_info = await self._credential_key_manager.generate_vdsnc_key(
            ...
            metadata.hsm_key_id = key_info.key_id
            await self._store_metadata(metadata)
            logger.info(
                "VDS-NC key %s backed by KMS (key_id=%s)", kid, key_info.key_id
            )
        elif not hsm_key_id:
...
async def example_key_lifecycle(session: AsyncSession) -> None:
    ...
    logger.info("Created key: %s", kid)
    logger.info("Valid from %s to %s", metadata.not_before, metadata.not_after)

    # 2. Check if rotation needed (after some time)
    if metadata.needs_rotation(warning_days=60):
        logger.info("Key needs rotation!")
    ...
    logger.info("Keys valid for verification: %d", len(valid_keys))
