...
def create_sod(
    ...
    certificate: x509.Certificate | CertificateBridge,
    ...
    *,
    ...
    Either ``private_key`` or ``sign_fn`` must be provided:
    - ``private_key``: local private key object (existing behavior).
    - ``sign_fn``: callable that signs raw bytes externally (BYOK/KMS).
      When using ``sign_fn``, ``signature_algorithm_name`` must also be
      provided (e.g. ``"sha256_rsa"``, ``"sha256_ecdsa"``, ``"ed25519"``).
    """
    if private_key is None and sign_fn is None:
        msg = "Either private_key or sign_fn must be provided"
        raise ValueError(msg)

    # Convert CertificateBridge to x509.Certificate if needed
    if isinstance(certificate, CertificateBridge):
    ...
    if sign_fn is not None:
        # External signer (BYOK/KMS path)
        if not signature_algorithm_name:
            msg = "signature_algorithm_name is required when using sign_fn"
            raise ValueError(msg)
        signature_algorithm = asn1_algos.SignedDigestAlgorithm(
            {"algorithm": signature_algorithm_name}
        )
        sign_payload = sign_fn
...
def verify_sod_signature(
    sod_bytes: bytes,
    trusted_certificates: Sequence[x509.Certificate | CertificateBridge] | None = None,
) -> bool:
