...
def _extract_auth_context(grpc_context: Any) -> dict[str, Any]:
    ...
    JWT tokens are validated using the signing key configured via
    ``MARTY_JWT_SECRET`` (HS256) or ``MARTY_JWT_PUBLIC_KEY`` (RS/ES).
    Falls back to client certificate subject if no JWT is present.
    """
    ...
    if auth_header.startswith("Bearer "):
        try:
            ...
            # Determine the validation key and algorithm
            jwt_public_key = os.environ.get("MARTY_JWT_PUBLIC_KEY")
            jwt_secret = os.environ.get("MARTY_JWT_SECRET")

            if jwt_public_key:
                if os.path.isfile(jwt_public_key):
                    key = Path(jwt_public_key).read_text()
                else:
                    key = jwt_public_key
                ...
                LOGGER.warning("No JWT validation key configured (MARTY_JWT_SECRET or MARTY_JWT_PUBLIC_KEY)")
                key = None
                algorithms = []
            ...
            if key:
                ...
                LOGGER.warning("JWT present but no validation key configured — token rejected")
