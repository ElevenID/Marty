...
def parse_mrz_simple(mrz_data: str) -> MRZData | None:
    ...
    try:
        ...
        if result.is_valid and result.parsed_data:
            # Filter parsed_data to only include MRZData fields
            mrz_fields = set(MRZData.model_fields.keys())
            filtered = {k: v for k, v in result.parsed_data.items() if k in mrz_fields}
            return MRZData(**filtered)
    except Exception:
