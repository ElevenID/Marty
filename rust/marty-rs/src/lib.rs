    ...
    match &jwk.params {
        Params::OKP(params) => {
            if let Some(d) = &params.private_key {
                SecretKey::new_ed25519(&d.0).map_err(|e| format!("Invalid Ed25519 key: {:?}", e))
            } else {
        ...
        Params::EC(params) => {
            if let Some(d) = &params.ecc_private_key {
                match params.curve.as_deref() {
                    Some("P-256") => {
                        SecretKey::new_p256(&d.0).map_err(|e| format!("Invalid P-256 key: {:?}", e))
                    }
...
fn verify_jwt(
    ...
    let payload: serde_json::Value = serde_json::from_slice(&payload_bytes)
        .map_err(|_| PyErr::new::<pyo3::exceptions::PyValueError, _>("Invalid JSON in payload"))?;
