...
pub fn generate_p256_key() -> Result<String, JsValue> {
    ...
    let encoded = base64::engine::general_purpose::URL_SAFE_NO_PAD
        .encode(jwk_str.as_bytes());
    let did = format!("did:jwk:{}", encoded);
    let key_id = format!("key_{}", uuid::Uuid::new_v4().to_string().replace("-", "")[..16].to_string());
...
pub fn create_credential_offer(
    ...
    if let Some(code) = pre_authorized_code {
        ...
        pre_auth_grant.insert("user_pin_required".to_string(), serde_json::json!(user_pin_required));
        grants.insert(
...
pub fn create_authorization_response(
    ...
    let presentation_submission: serde_json::Value = serde_json::from_str(presentation_submission_json)
        ...
        Params::EC(params) => {
            if let Some(d) = &params.ecc_private_key {
                match params.curve.as_deref() {
                    ...
                    curve => return Err(JsValue::from_str(&format!("Unsupported curve: {:?}", curve))),
                }
            } else {
