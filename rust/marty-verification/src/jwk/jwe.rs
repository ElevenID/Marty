...
pub fn jwe_encrypt_direct(
    ...
    let (epk_public, shared_secret) =
        match (recipient_key.kty.as_str(), recipient_key.crv.as_deref()) {
            ("OKP", Some("X25519")) => {
                use crate::crypto::ecdh::x25519_ephemeral_agree;
    ...
    // Encrypt content
    use crate::crypto::symmetric::{aes_128_gcm_encrypt, aes_256_gcm_encrypt};
...
pub fn jwe_decrypt(jwe: &str, recipient_key: &Jwk) -> VerificationResult<Vec<u8>> {
    ...
    let shared_secret = match header.alg.as_str() {
        "ECDH-ES" | "ECDH-ES+A128KW" | "ECDH-ES+A256KW" => {
            ...
            match (recipient_key.kty.as_str(), recipient_key.crv.as_deref()) {
                ("OKP", Some("X25519")) => {
                    use crate::crypto::ecdh::X25519KeyPair;
    ...
    // Decrypt
    use crate::crypto::symmetric::{aes_128_gcm_decrypt, aes_256_gcm_decrypt};
