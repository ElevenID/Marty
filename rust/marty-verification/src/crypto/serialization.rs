...
pub fn load_private_key_pem(pem_data: &str) -> VerificationResult<Vec<u8>> {
    // Check for encrypted key
    if pem_data.contains("ENCRYPTED") {
        return Err(VerificationError::key_error(
    ...
    if pem_data.contains("BEGIN PRIVATE KEY") {
        ...
        let (label, doc) = pem_rfc7468::decode_vec(pem_data.as_bytes()).map_err(|e| {
            VerificationError::pem_error(format!("Failed to parse PKCS#8 PEM: {}", e))
        })?;
        if label != "PRIVATE KEY" {
            return Err(VerificationError::pem_error("Expected PRIVATE KEY label"));
        }
        return Ok(doc);
    }
    ...
    Err(VerificationError::pem_error("Unknown private key format"))
}

/// Load an EC private key from SEC1 PEM format.
fn load_ec_private_key_sec1_pem(pem_data: &str) -> VerificationResult<Vec<u8>> {
    // Try P-256 first
    if let Ok(key) = p256::SecretKey::from_sec1_pem(pem_data) {
        use pkcs8::EncodePrivateKey;
        let doc = key.to_pkcs8_der().map_err(|e| {
            VerificationError::encoding_error(format!("Failed to convert to PKCS#8: {}", e))
        })?;
    ...
    Err(VerificationError::pem_error(
        "Failed to parse SEC1 EC private key",
    ))
}

/// Load an RSA private key from PKCS#1 PEM format.
fn load_rsa_private_key_pkcs1_pem(pem_data: &str) -> VerificationResult<Vec<u8>> {
    use pkcs8::EncodePrivateKey;
    use rsa::pkcs1::DecodeRsaPrivateKey;

    let key = rsa::RsaPrivateKey::from_pkcs1_pem(pem_data).map_err(|e| {
        VerificationError::pem_error(format!("Failed to parse PKCS#1 RSA key: {}", e))
    })?;
...
pub fn load_private_key_der(der_data: &[u8]) -> VerificationResult<Vec<u8>> {
    // Validate it's a valid PKCS#8 structure
    let _info = pkcs8::PrivateKeyInfo::from_der(der_data).map_err(|e| {
        VerificationError::der_error(format!("Failed to parse private key DER: {}", e))
    })?;
...
pub fn save_private_key_pem(private_key_der: &[u8]) -> VerificationResult<String> {
    use base64::Engine;
...
pub fn save_private_key_der(private_key_der: &[u8]) -> VerificationResult<Vec<u8>> {
...
pub fn raw_private_key_to_pkcs8(raw_key: &[u8], key_type: &str) -> VerificationResult<Vec<u8>> {
    match key_type {
        "EC_P256" | "P256" | "secp256r1" => {
            ...
            let array: [u8; 32] = raw_key
                .try_into()
                .map_err(|_| VerificationError::key_error("Invalid P-256 key length"))?;
            let secret = p256::SecretKey::from_bytes(&array.into())
                .map_err(|e| VerificationError::key_error(format!("Invalid P-256 key: {}", e)))?;
            use pkcs8::EncodePrivateKey;
            let doc = secret.to_pkcs8_der().map_err(|e| {
                VerificationError::encoding_error(format!("Failed to encode PKCS#8: {}", e))
            })?;
            Ok(doc.as_bytes().to_vec())
        }
        "EC_P384" | "P384" | "secp384r1" => {
            ...
            let array: [u8; 48] = raw_key
                .try_into()
                .map_err(|_| VerificationError::key_error("Invalid P-384 key length"))?;
            let secret = p384::SecretKey::from_bytes(&array.into())
                .map_err(|e| VerificationError::key_error(format!("Invalid P-384 key: {}", e)))?;
            use pkcs8::EncodePrivateKey;
            let doc = secret.to_pkcs8_der().map_err(|e| {
                VerificationError::encoding_error(format!("Failed to encode PKCS#8: {}", e))
            })?;
        ...
        "Ed25519" => {
            ...
            let array: [u8; 32] = raw_key
                .try_into()
                .map_err(|_| VerificationError::key_error("Invalid Ed25519 key length"))?;
...
/// * `key_type` - One of "EC_P256", "EC_P384", "Ed25519"
pub fn raw_public_key_to_spki(raw_key: &[u8], key_type: &str) -> VerificationResult<Vec<u8>> {
    match key_type {
        "EC_P256" | "P256" | "secp256r1" => {
            let point = p256::EncodedPoint::from_bytes(raw_key).map_err(|e| {
                VerificationError::key_error(format!("Invalid P-256 public key: {}", e))
            })?;
            let public_key = p256::PublicKey::from_encoded_point(&point)
                .into_option()
                .ok_or_else(|| VerificationError::key_error("Invalid P-256 public key point"))?;
            use pkcs8::EncodePublicKey;
            let doc = public_key.to_public_key_der().map_err(|e| {
                VerificationError::encoding_error(format!("Failed to encode SPKI: {}", e))
            })?;
            Ok(doc.as_bytes().to_vec())
        }
        "EC_P384" | "P384" | "secp384r1" => {
            let point = p384::EncodedPoint::from_bytes(raw_key).map_err(|e| {
                VerificationError::key_error(format!("Invalid P-384 public key: {}", e))
            })?;
            let public_key = p384::PublicKey::from_encoded_point(&point)
                .into_option()
                .ok_or_else(|| VerificationError::key_error("Invalid P-384 public key point"))?;
            use pkcs8::EncodePublicKey;
            let doc = public_key.to_public_key_der().map_err(|e| {
                VerificationError::encoding_error(format!("Failed to encode SPKI: {}", e))
            })?;
...
pub fn pkcs8_to_raw_private_key(pkcs8_der: &[u8]) -> VerificationResult<(Vec<u8>, String)> {
    ...
    match key_type.as_str() {
        "EC_P256" => {
            ...
            let secret = p256::SecretKey::from_pkcs8_der(pkcs8_der).map_err(|e| {
                VerificationError::key_error(format!("Failed to parse P-256 key: {}", e))
            })?;
        ...
        "EC_P384" => {
            ...
            let secret = p384::SecretKey::from_pkcs8_der(pkcs8_der).map_err(|e| {
                VerificationError::key_error(format!("Failed to parse P-384 key: {}", e))
            })?;
            Ok((secret.to_bytes().to_vec(), key_type))
        }
        "Ed25519" => {
            let info = pkcs8::PrivateKeyInfo::from_der(pkcs8_der)
                .map_err(|e| VerificationError::der_error(format!("Failed to parse key: {}", e)))?;
...
pub fn spki_to_raw_public_key(spki_der: &[u8]) -> VerificationResult<(Vec<u8>, String)> {
    let key_type = detect_public_key_type(spki_der)?;
    let info = SubjectPublicKeyInfoOwned::from_der(spki_der)
        .map_err(|e| VerificationError::der_error(format!("Failed to parse public key: {}", e)))?;

    let raw_bytes = info
        .subject_public_key
        .as_bytes()
        .ok_or_else(|| VerificationError::key_error("Invalid public key bit string"))?;
...
pub fn load_public_key_pem(pem_data: &str) -> VerificationResult<Vec<u8>> {
    let info = SubjectPublicKeyInfoOwned::from_pem(pem_data).map_err(|e| {
        VerificationError::pem_error(format!("Failed to parse public key PEM: {}", e))
    })?;

    info.to_der().map_err(|e| {
        VerificationError::encoding_error(format!("Failed to encode public key: {}", e))
    })
}

/// Load a public key from DER format (SPKI).
pub fn load_public_key_der(der_data: &[u8]) -> VerificationResult<Vec<u8>> {
    // Validate it's a valid SPKI structure
    let _info = SubjectPublicKeyInfoOwned::from_der(der_data).map_err(|e| {
        VerificationError::der_error(format!("Failed to parse public key DER: {}", e))
    })?;
...
pub fn save_public_key_pem(public_key_der: &[u8]) -> VerificationResult<String> {
    use base64::Engine;
...
pub fn extract_public_key(private_key_der: &[u8]) -> VerificationResult<Vec<u8>> {
    ...
    match key_type.as_str() {
        ...
        "RSA" => {
            ...
            let private = rsa::RsaPrivateKey::from_pkcs8_der(private_key_der).map_err(|e| {
                VerificationError::key_error(format!("Failed to parse RSA key: {}", e))
            })?;
        ...
        "Ed25519" => {
            ...
            if private_bytes.len() >= 32 {
                ...
                Err(VerificationError::key_error("Invalid Ed25519 private key"))
            }
        }
        _ => Err(VerificationError::key_error(format!(
            "Unsupported key type: {}",
...
fn encode_ed25519_public_key_spki(public_key: &[u8]) -> VerificationResult<Vec<u8>> {
...
pub fn detect_private_key_type(der_data: &[u8]) -> VerificationResult<String> {
    let info = pkcs8::PrivateKeyInfo::from_der(der_data)
        .map_err(|e| VerificationError::der_error(format!("Failed to parse private key: {}", e)))?;
...
pub fn detect_public_key_type(der_data: &[u8]) -> VerificationResult<String> {
...
pub fn get_key_size(public_key_der: &[u8]) -> VerificationResult<usize> {
    ...
    match key_type.as_str() {
        ...
        "RSA" => {
            ...
            let key_bytes = info
                .subject_public_key
                .as_bytes()
                .ok_or_else(|| VerificationError::key_error("Invalid RSA public key"))?;
...
mod tests {
    ...
    fn test_pem_roundtrip_private_key() {
        ...
        let generated =
            crate::crypto::keygen::generate_keypair(crate::crypto::keygen::KeyType::EcdsaP256)
                .unwrap();
