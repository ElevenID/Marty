...
pub fn parse_pkcs12(data: &[u8], password: &str) -> VerificationResult<Pkcs12Data> {
    // Use the p12 crate to parse
    let p12 = p12::PFX::parse(data).map_err(|e| {
        VerificationError::crypto_error(format!("Failed to parse PKCS#12: {:?}", e))
    })?;

    // Get the key bags (private keys) - returns Vec<Vec<u8>>
    let key_bags = p12.key_bags(password).map_err(|e| {
        VerificationError::crypto_error(format!("Failed to decrypt PKCS#12 key: {:?}", e))
    })?;

    if key_bags.is_empty() {
        return Err(VerificationError::crypto_error(
    ...
    let cert_bags = p12.cert_x509_bags(password).map_err(|e| {
        VerificationError::crypto_error(format!("Failed to decrypt PKCS#12 certificates: {:?}", e))
    })?;
...
pub fn parse_pkcs12_private_key_pem(data: &[u8], password: &str) -> VerificationResult<String> {
    ...
    .map_err(|e| VerificationError::crypto_error(format!("Failed to encode PEM: {}", e)))?;
...
pub fn parse_pkcs12_certificate_pem(data: &[u8], password: &str) -> VerificationResult<String> {
    let parsed = parse_pkcs12(data, password)?;
...
pub fn parse_pkcs12_chain_pem(data: &[u8], password: &str) -> VerificationResult<Vec<String>> {
    let parsed = parse_pkcs12(data, password)?;
...
fn detect_key_algorithm(pkcs8_der: &[u8]) -> VerificationResult<PrivateKeyAlgorithm> {
    use pkcs8::PrivateKeyInfo;

    let pki = PrivateKeyInfo::from_der(pkcs8_der)
        .map_err(|e| VerificationError::crypto_error(format!("Failed to parse PKCS#8: {}", e)))?;
