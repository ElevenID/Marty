...
pub fn iso9796_verify(
    ...
    let public_key = RsaPublicKey::from_public_key_der(public_key_der).map_err(|e| {
        VerificationError::crypto_error(format!("Failed to parse RSA public key: {}", e))
    })?;
...
pub fn iso9796_recover_message(
    ...
    if signature.len() != k {
        return Err(VerificationError::crypto_error("Invalid signature length"));
    }
    ...
    match scheme {
        ...
        Iso9796Scheme::Scheme2 | Iso9796Scheme::Scheme3 => {
            let hash_alg = hash_alg.ok_or_else(|| {
                VerificationError::crypto_error("Hash algorithm required for Scheme 2/3")
            })?;
