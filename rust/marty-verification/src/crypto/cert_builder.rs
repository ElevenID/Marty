...
) -> VerificationResult<x509_cert::time::Time> {
    use der::asn1::GeneralizedTime;

    let gt = GeneralizedTime::from_unix_duration(duration)
        .map_err(|e| VerificationError::internal(format!("Invalid time: {}", e)))?;
...
impl DistinguishedName {
    ...
    pub fn build(&self) -> VerificationResult<Name> {
        ...
        Name::from_der(
            &der::asn1::PrintableString::new(&name_str)
                .map_err(|e| VerificationError::internal(format!("Invalid name string: {}", e)))?
                .to_der()
                .map_err(|e| {
                    VerificationError::internal(format!("Failed to encode name: {}", e))
                })?,
        )
        .map_err(|e| VerificationError::internal(format!("Failed to parse name: {}", e)))
    }
}
...
impl CertificateBuilderConfig {
    ...
    pub fn build_self_signed(&self) -> VerificationResult<(Vec<u8>, String)> {
        match self.key_type {
    ...
    pub fn build_self_signed_with_key(&self, private_key_pem: &str) -> VerificationResult<Vec<u8>> {
    ...
    fn build_self_signed_with_p256_key(
        ...
        let signing_key = P256SigningKey::from_pkcs8_pem(private_key_pem).map_err(|e| {
            VerificationError::internal(format!("Failed to parse P-256 key: {}", e))
        })?;
    ...
    fn build_self_signed_with_p384_key(
        ...
        let signing_key = P384SigningKey::from_pkcs8_pem(private_key_pem).map_err(|e| {
            VerificationError::internal(format!("Failed to parse P-384 key: {}", e))
        })?;
        let verifying_key = signing_key.verifying_key();
        let public_key_bytes = verifying_key.to_sec1_bytes().to_vec();
    ...
    fn build_self_signed_with_rsa_key(&self, private_key_pem: &str) -> VerificationResult<Vec<u8>> {
        use rsa::{pkcs8::DecodePrivateKey, RsaPrivateKey};

        let private_key = RsaPrivateKey::from_pkcs8_pem(private_key_pem)
            .map_err(|e| VerificationError::internal(format!("Failed to parse RSA key: {}", e)))?;
    ...
    fn build_self_signed_with_ed25519_key(
        ...
        let signing_key = Ed25519SigningKey::from_pkcs8_pem(private_key_pem).map_err(|e| {
            VerificationError::internal(format!("Failed to parse Ed25519 key: {}", e))
        })?;
        let verifying_key = signing_key.verifying_key();
    ...
    fn build_self_signed_p256(&self) -> VerificationResult<(Vec<u8>, String)> {
        ...
        let private_key_pem = signing_key
            ...
            .map_err(|e| {
                VerificationError::internal(format!("Failed to encode private key: {}", e))
            })?
            .to_string();
    ...
    fn build_self_signed_p384(&self) -> VerificationResult<(Vec<u8>, String)> {
        use p384::ecdsa::SigningKey as P384SigningKey;
    ...
    fn build_self_signed_rsa(&self) -> VerificationResult<(Vec<u8>, String)> {
        ...
        let private_key = RsaPrivateKey::new(&mut OsRng, bits).map_err(|e| {
            VerificationError::internal(format!("Failed to generate RSA key: {}", e))
        })?;
    ...
    fn build_self_signed_ed25519(&self) -> VerificationResult<(Vec<u8>, String)> {
        ...
        let private_key_pem = signing_key
            .to_pkcs8_pem(Default::default())
            .map_err(|e| {
                VerificationError::internal(format!("Failed to encode Ed25519 private key: {}", e))
            })?
            .to_string();
    ...
    fn build_signed_by_p256(
        ...
        let issuer_signing_key = P256SigningKey::from_pkcs8_pem(issuer_key_pem).map_err(|e| {
            VerificationError::internal(format!("Failed to parse P-256 issuer key: {}", e))
        })?;
    ...
    fn build_signed_by_p384(
        ...
        let issuer_signing_key = P384SigningKey::from_pkcs8_pem(issuer_key_pem).map_err(|e| {
            VerificationError::internal(format!("Failed to parse P-384 issuer key: {}", e))
        })?;
    ...
    fn build_signed_by_rsa(
        ...
        let issuer_signing_key = RsaPrivateKey::from_pkcs8_pem(issuer_key_pem).map_err(|e| {
            VerificationError::internal(format!("Failed to parse RSA issuer key: {}", e))
        })?;
    ...
    fn build_signed_by_ed25519(
        ...
        let issuer_signing_key =
            Ed25519SigningKey::from_pkcs8_pem(issuer_key_pem).map_err(|e| {
                VerificationError::internal(format!("Failed to parse Ed25519 issuer key: {}", e))
            })?;
    ...
    fn build_certificate_with_ecdsa_p256(
        ...
        let builder = X509CertBuilder::new(profile, serial, validity, subject, spki, &signer)
            .map_err(|e| {
                VerificationError::internal(format!("Failed to create cert builder: {}", e))
            })?;

        let cert = builder.build().map_err(|e| {
            VerificationError::internal(format!("Failed to build certificate: {}", e))
        })?;

        cert.to_der().map_err(|e| {
            VerificationError::internal(format!("Failed to encode certificate: {}", e))
        })
    }
    ...
    fn build_certificate_with_rsa(
        ...
        let spki = SubjectPublicKeyInfoOwned::from_der(spki_der.as_bytes())
            .map_err(|e| VerificationError::internal(format!("Failed to parse SPKI: {}", e)))?;
    ...
    fn build_name(&self, dn: &DistinguishedName) -> VerificationResult<Name> {
        use std::str::FromStr;
        ...
        Name::from_str(&name_str).map_err(|e| {
            VerificationError::internal(format!("Failed to parse name '{}': {}", name_str, e))
        })
    }

    fn create_validity(&self) -> VerificationResult<Validity> {
        use std::time::{Duration, SystemTime, UNIX_EPOCH};

        let now = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .map_err(|_| VerificationError::internal("System time error"))?;
    ...
    fn generate_serial_number(&self) -> VerificationResult<SerialNumber> {
        use rand::RngCore;
        ...
        SerialNumber::new(&bytes)
            .map_err(|e| VerificationError::internal(format!("Invalid serial number: {}", e)))
    }

    fn build_ecdsa_p256_spki(
        &self,
        public_key_bytes: &[u8],
    ) -> VerificationResult<SubjectPublicKeyInfoOwned> {
        ...
        let ec_public_key_oid = ObjectIdentifier::new("1.2.840.10045.2.1")
            .map_err(|e| VerificationError::internal(format!("Invalid OID: {}", e)))?;
        let secp256r1_oid = ObjectIdentifier::new("1.2.840.10045.3.1.7")
            .map_err(|e| VerificationError::internal(format!("Invalid curve OID: {}", e)))?;

        // Build parameters (curve OID)
        let params = der::Any::from_der(&secp256r1_oid.to_der().map_err(|e| {
            VerificationError::internal(format!("Failed to encode curve OID: {}", e))
        })?)
        .map_err(|e| VerificationError::internal(format!("Failed to parse curve OID: {}", e)))?;
        ...
        let subject_public_key = BitString::from_bytes(&point_bytes).map_err(|e| {
            VerificationError::internal(format!("Failed to create bit string: {}", e))
        })?;
    ...
    fn build_ed25519_spki(
        ...
        let ed25519_oid = ObjectIdentifier::new("1.3.101.112")
            .map_err(|e| VerificationError::internal(format!("Invalid Ed25519 OID: {}", e)))?;
...
pub fn create_mock_certificate(
    ...
    let serial = hex::decode(serial_number_hex)
        .map_err(|e| VerificationError::internal(format!("Invalid serial number hex: {}", e)))?;
...
impl CsrBuilderConfig {
    ...
    pub fn build(&self) -> VerificationResult<(Vec<u8>, String)> {
        match self.key_type {
    ...
    fn build_csr_p256(&self) -> VerificationResult<(Vec<u8>, String)> {
        // Generate P-256 key pair
        let signing_key = P256SigningKey::random(&mut OsRng);
        ...
        let builder = RequestBuilder::new(subject, &signing_key).map_err(|e| {
            VerificationError::internal(format!("Failed to create CSR builder: {}", e))
        })?;

        let csr: CertReq = builder
            .build::<p256::ecdsa::DerSignature>()
            .map_err(|e| VerificationError::internal(format!("Failed to build CSR: {}", e)))?;

        let csr_der = csr
            .to_der()
            .map_err(|e| VerificationError::internal(format!("Failed to encode CSR: {}", e)))?;

        Ok((csr_der, private_key_pem))
    }

    fn build_csr_p384(&self) -> VerificationResult<(Vec<u8>, String)> {
        use p384::ecdsa::SigningKey as P384SigningKey;
        use p384::pkcs8::EncodePrivateKey as _;
    ...
    fn build_csr_rsa(&self) -> VerificationResult<(Vec<u8>, String)> {
    ...
    fn build_csr_ed25519(&self) -> VerificationResult<(Vec<u8>, String)> {
        use ed25519_dalek::pkcs8::EncodePrivateKey as _;
        use rand::RngCore;
    ...
    fn build_name(&self) -> VerificationResult<Name> {
        use std::str::FromStr;
...
pub fn create_csr(common_name: &str, key_type: KeyType) -> VerificationResult<(Vec<u8>, String)> {
...
pub fn load_csr_der(csr_der: &[u8]) -> VerificationResult<CsrInfo> {
    let csr = CertReq::from_der(csr_der)
        .map_err(|e| VerificationError::der_error(format!("Failed to parse CSR: {}", e)))?;
...
mod tests {
    use super::*;
    use crate::crypto::certificate::get_certificate_info;
