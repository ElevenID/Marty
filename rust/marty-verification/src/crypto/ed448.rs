...
pub fn ed448_generate() -> VerificationResult<(Vec<u8>, Vec<u8>)> {
...
/// # Returns
/// 114-byte signature
pub fn ed448_sign(private_key: &[u8], message: &[u8]) -> VerificationResult<Vec<u8>> {
    if private_key.len() != ED448_PRIVATE_KEY_SIZE {
        return Err(VerificationError::crypto_error(format!(
    ...
    let signing_key = SigningKey::try_from(private_key)
        .map_err(|e| VerificationError::crypto_error(format!("Invalid private key: {}", e)))?;
...
pub fn ed448_sign_with_context(
    ...
    let signature = signing_key
        ...
        .map_err(|e| VerificationError::crypto_error(format!("Signing failed: {:?}", e)))?;
...
pub fn ed448_verify(
    ...
    let pk_array: [u8; 57] = public_key
        .try_into()
        .map_err(|_| VerificationError::crypto_error("Invalid public key length"))?;
    ...
    let sig_array: [u8; 114] = signature
        .try_into()
        .map_err(|_| VerificationError::crypto_error("Invalid signature length"))?;
