...
impl OcspRequestBuilder {
    ...
    pub fn build(&self) -> VerificationResult<Vec<u8>> {
        ...
        ocsp_request.to_der().map_err(|e| {
            VerificationError::internal(format!("Failed to encode OCSP request: {}", e))
        })
    }

    /// Build CertID for a certificate/issuer pair.
    fn build_cert_id(&self, cert_der: &[u8], issuer_der: &[u8]) -> VerificationResult<CertId> {
        use der::asn1::OctetString;
        use spki::AlgorithmIdentifierOwned;

        // Parse certificates
        let cert = Certificate::from_der(cert_der).map_err(|e| {
            VerificationError::der_error(format!("Failed to parse certificate: {}", e))
        })?;
        ...
        let issuer_name_der = issuer.tbs_certificate.subject.to_der().map_err(|e| {
            VerificationError::internal(format!("Failed to encode issuer name: {}", e))
        })?;
        ...
        Ok(CertId {
            ...
            issuer_name_hash: OctetString::new(issuer_name_hash.to_vec()).map_err(|e| {
                VerificationError::internal(format!("Failed to create octet string: {}", e))
            })?,
...
/// Parse an OCSP response.
pub fn parse_ocsp_response(response_der: &[u8]) -> VerificationResult<OcspResponseInfo> {
    let response = OcspResponse::from_der(response_der).map_err(|e| {
        VerificationError::der_error(format!("Failed to parse OCSP response: {}", e))
    })?;
    ...
    let response_bytes = response.response_bytes.ok_or_else(|| {
        VerificationError::internal("Missing response bytes in successful OCSP response")
    })?;
    ...
    let basic_response =
        BasicOcspResponse::from_der(response_bytes.response.as_bytes()).map_err(|e| {
            VerificationError::der_error(format!("Failed to parse BasicOCSPResponse: {}", e))
        })?;
    ...
    let single_response = tbs
        .responses
        .first()
        .ok_or_else(|| VerificationError::internal("No single responses in OCSP response"))?;
...
pub fn is_revoked_via_ocsp(response_der: &[u8]) -> VerificationResult<bool> {
    let response = parse_ocsp_response(response_der)?;
    ...
    match response.cert_status {
        ...
        Some(OcspCertStatus::Unknown) => {
            Err(VerificationError::internal("OCSP returned unknown status"))
        }
        None => Err(VerificationError::internal(
...
impl OcspResponseBuilder {
    ...
    pub fn build(&self, responder_key_pem: &str) -> VerificationResult<Vec<u8>> {
        ...
        let cert_der = self
            .cert_der
            .as_ref()
            .ok_or_else(|| VerificationError::internal("Certificate required for OCSP response"))?;
        let issuer_der = self.issuer_der.as_ref().ok_or_else(|| {
            VerificationError::internal("Issuer certificate required for OCSP response")
        })?;
            ...
            if let Ok(signing_key) = P256SigningKey::from_pkcs8_pem(responder_key_pem) {
                ...
                let sig_alg = spki::AlgorithmIdentifierOwned {
                    oid: der::asn1::ObjectIdentifier::new("1.2.840.10045.4.3.2")
                        .map_err(|_| VerificationError::internal("Invalid OID"))?,
                    parameters: None,
                };
                (sig.as_bytes().to_vec(), sig_alg)
            } else {
                return Err(VerificationError::internal("Unable to parse responder key"));
            };

        let sig_bits = der::asn1::BitString::from_bytes(&signature).map_err(|e| {
            VerificationError::internal(format!("Failed to create signature: {}", e))
        })?;
        ...
        let basic_response_der = basic_response.to_der().map_err(|e| {
            VerificationError::internal(format!("Failed to encode BasicOCSPResponse: {}", e))
        })?;
        ...
        let response_bytes = x509_ocsp::ResponseBytes {
            ...
            response: OctetString::new(basic_response_der).map_err(|e| {
                VerificationError::internal(format!("Failed to create response bytes: {}", e))
            })?,
        };
        ...
        ocsp_response.to_der().map_err(|e| {
            VerificationError::internal(format!("Failed to encode OCSP response: {}", e))
        })
    }
}

/// Build a simple OCSP request for a certificate.
pub fn build_ocsp_request(cert_der: &[u8], issuer_cert_der: &[u8]) -> VerificationResult<Vec<u8>> {
...
pub async fn ocsp_check(
    ...
    issuer_cert_der: &[u8],
    responder_url: &str,
) -> VerificationResult<OcspCheckResult> {
    use base64::Engine;
    ...
    let response = client
        ...
        .send()
        .await
        .map_err(|e| VerificationError::network_error(format!("OCSP request failed: {}", e)))?;
    ...
    let response_bytes = response.bytes().await.map_err(|e| {
        VerificationError::network_error(format!("Failed to read OCSP response: {}", e))
    })?;
...
pub async fn ocsp_check_get(
    ...
    let response = client
        ...
        .send()
        .await
        .map_err(|e| VerificationError::network_error(format!("OCSP GET request failed: {}", e)))?;
...
pub fn get_ocsp_responder_url(cert_der: &[u8]) -> VerificationResult<Option<String>> {
    let cert = Certificate::from_der(cert_der)
        .map_err(|e| VerificationError::der_error(format!("Failed to parse certificate: {}", e)))?;
...
impl OcspCacheKey {
    ...
    pub fn from_certs(cert_der: &[u8], issuer_der: &[u8]) -> VerificationResult<Self> {
        let cert = Certificate::from_der(cert_der).map_err(|e| {
            VerificationError::der_error(format!("Failed to parse certificate: {}", e))
        })?;
        let issuer = Certificate::from_der(issuer_der).map_err(|e| {
...
impl OcspClient {
    ...
    pub async fn check(
    ...
    ) -> VerificationResult<(OcspCheckResult, bool)> {
        // Build cache key
        ...
        let url = match responder_url {
            ...
            None => get_ocsp_responder_url(cert_der)?.ok_or_else(|| {
                VerificationError::internal("No OCSP responder URL found in certificate")
            })?,
        };
    ...
    async fn check_via_get(
        ...
        let response = self
            ...
            .await
            .map_err(|e| {
                VerificationError::network_error(format!("OCSP GET request failed: {}", e))
            })?;
...
mod tests_with_cert_builder {
    use super::*;
    use crate::crypto::cert_builder::{create_ca_certificate, create_signed_certificate};
