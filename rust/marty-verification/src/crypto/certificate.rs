...
pub fn load_certificate_pem(pem_data: &str) -> VerificationResult<Vec<u8>> {
    let cert = Certificate::from_pem(pem_data).map_err(|e| {
        VerificationError::pem_error(format!("Failed to parse PEM certificate: {}", e))
    })?;

    cert.to_der()
        .map_err(|e| VerificationError::der_error(format!("Failed to encode certificate: {}", e)))
}

/// Load a certificate from DER-encoded data and validate it.
pub fn load_certificate_der(der_data: &[u8]) -> VerificationResult<Certificate> {
    Certificate::from_der(der_data).map_err(|e| {
        VerificationError::der_error(format!("Failed to parse DER certificate: {}", e))
    })
}

/// Extract information from a DER-encoded certificate.
pub fn get_certificate_info(der_data: &[u8]) -> VerificationResult<CertificateInfo> {
    ...
    let fingerprint_bytes = crate::crypto::hashing::hash_sha256(der_data);
...
pub fn pem_to_der(pem_data: &str) -> VerificationResult<Vec<u8>> {
    load_certificate_pem(pem_data)
}

/// Convert DER certificate to PEM.
pub fn der_to_pem(der_data: &[u8]) -> VerificationResult<String> {
    use base64::Engine;
...
pub fn get_certificate_public_key(der_data: &[u8]) -> VerificationResult<Vec<u8>> {
    let cert = load_certificate_der(der_data)?;

    cert.tbs_certificate
        .subject_public_key_info
        .to_der()
        .map_err(|e| VerificationError::der_error(format!("Failed to encode public key: {}", e)))
}

/// Check if a certificate is expired.
pub fn is_certificate_expired(der_data: &[u8]) -> VerificationResult<bool> {
    let cert = load_certificate_der(der_data)?;
...
pub fn is_certificate_not_yet_valid(der_data: &[u8]) -> VerificationResult<bool> {
    let cert = load_certificate_der(der_data)?;
    let now = chrono::Utc::now();
...
) -> VerificationResult<chrono::DateTime<chrono::Utc>> {
    let time_str = format_x509_time(time);

    // Parse ISO 8601 format
    chrono::DateTime::parse_from_rfc3339(&time_str)
        .map(|dt| dt.with_timezone(&chrono::Utc))
        .map_err(|e| VerificationError::parse_error(format!("Failed to parse time: {}", e)))
}
...
pub fn verify_certificate_signature(
    ...
    let tbs_der = cert
        .tbs_certificate
        .to_der()
        .map_err(|e| VerificationError::der_error(format!("Failed to encode TBS: {}", e)))?;
    ...
    let signature = cert
        .signature
        .as_bytes()
        .ok_or_else(|| VerificationError::signature_error("Invalid signature bits"))?;
    ...
    let issuer_pubkey_der = issuer
        ...
        .to_der()
        .map_err(|e| {
            VerificationError::der_error(format!("Failed to encode issuer public key: {}", e))
        })?;

    // Convert OID to signature algorithm enum
    let oid_str = sig_alg.oid.to_string();
    let algorithm = crate::crypto::SignatureAlgorithm::from_oid(&oid_str)?;

    crate::crypto::verify_signature(algorithm, &issuer_pubkey_der, &tbs_der, signature)
}
