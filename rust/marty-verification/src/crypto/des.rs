...
/// ```
pub fn tdes_cbc_encrypt(key: &[u8], iv: &[u8], plaintext: &[u8]) -> VerificationResult<Vec<u8>> {
    validate_tdes_params(key, iv, plaintext)?;

    let encryptor = TdesEncryptor::new_from_slices(key, iv)
        .map_err(|e| VerificationError::internal(format!("3DES key/IV error: {}", e)))?;
    ...
    let ciphertext = encryptor
        ...
        .map_err(|e| VerificationError::internal(format!("3DES-CBC encryption failed: {}", e)))?;
...
///
/// Decrypted plaintext.
pub fn tdes_cbc_decrypt(key: &[u8], iv: &[u8], ciphertext: &[u8]) -> VerificationResult<Vec<u8>> {
    ...
    let plaintext = decryptor
        ...
        .map_err(|e| VerificationError::internal(format!("3DES-CBC decryption failed: {}", e)))?;
...
///
/// 8-byte MAC value.
pub fn retail_mac(key: &[u8], data: &[u8]) -> VerificationResult<Vec<u8>> {
    if key.len() != 16 {
        return Err(VerificationError::internal(
...
fn validate_tdes_params(key: &[u8], iv: &[u8], data: &[u8]) -> VerificationResult<()> {
    if key.len() != 24 {
        return Err(VerificationError::internal(
            "3DES requires 24-byte (192-bit) key".to_string(),
        ));
