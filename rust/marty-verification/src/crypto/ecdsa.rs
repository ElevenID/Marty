...
pub fn generate_p256_keypair() -> VerificationResult<(Vec<u8>, Vec<u8>)> {
...
pub fn generate_p384_keypair() -> VerificationResult<(Vec<u8>, Vec<u8>)> {
...
pub fn generate_p521_keypair() -> VerificationResult<(Vec<u8>, Vec<u8>)> {
...
pub fn sign_p256_sha256(private_key: &[u8], message: &[u8]) -> VerificationResult<Vec<u8>> {
    ...
    let signing_key = P256SigningKey::from_slice(private_key)
        .map_err(|e| VerificationError::internal(format!("Invalid P-256 private key: {}", e)))?;
...
///
/// DER-encoded signature.
pub fn sign_p384_sha384(private_key: &[u8], message: &[u8]) -> VerificationResult<Vec<u8>> {
    ...
    let signing_key = P384SigningKey::from_slice(private_key)
        .map_err(|e| VerificationError::internal(format!("Invalid P-384 private key: {}", e)))?;
...
///
/// DER-encoded signature.
pub fn sign_p521_sha512(private_key: &[u8], message: &[u8]) -> VerificationResult<Vec<u8>> {
    ...
    let signing_key = P521SigningKey::from_slice(private_key)
        .map_err(|e| VerificationError::internal(format!("Invalid P-521 private key: {}", e)))?;
...
pub fn extract_ec_point_from_spki(spki_der: &[u8]) -> VerificationResult<Vec<u8>> {
    use der::Decode;
    use x509_cert::spki::SubjectPublicKeyInfoOwned;

    let spki = SubjectPublicKeyInfoOwned::from_der(spki_der)
        .map_err(|e| VerificationError::der_error(format!("Failed to parse SPKI: {}", e)))?;
