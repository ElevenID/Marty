...
pub fn aes_128_gcm_encrypt(
    ...
    let cipher = Aes128Gcm::new_from_slice(key)
        .map_err(|e| VerificationError::internal(format!("AES key error: {}", e)))?;
    ...
    ciphertext.map_err(|e| VerificationError::internal(format!("AES-GCM encryption failed: {}", e)))
}
...
pub fn aes_128_gcm_decrypt(
    ...
    plaintext.map_err(|_| {
        VerificationError::internal("AES-GCM decryption failed: authentication failed".to_string())
    })
}
...
///
/// Padded ciphertext.
pub fn aes_128_cbc_encrypt(key: &[u8], iv: &[u8], plaintext: &[u8]) -> VerificationResult<Vec<u8>> {
    if key.len() != 16 {
        return Err(VerificationError::internal(
    ...
    let cipher = Aes128CbcEnc::new_from_slices(key, iv)
        .map_err(|e| VerificationError::internal(format!("AES key/IV error: {}", e)))?;
    ...
    let ciphertext = cipher
        ...
        .map_err(|e| VerificationError::internal(format!("AES-CBC encryption failed: {}", e)))?;
...
pub fn aes_128_cbc_decrypt(
    ...
    let plaintext = cipher
        ...
        .map_err(|_| {
            VerificationError::internal("AES-CBC decryption failed: invalid padding".to_string())
        })?;
...
pub fn aes_128_cbc_decrypt_nopad(
    ...
    cipher
        ...
        .map_err(|_| VerificationError::internal("AES-CBC decryption failed".to_string()))?;
...
pub fn aes_128_cmac(key: &[u8], data: &[u8]) -> VerificationResult<Vec<u8>> {
    use cmac::{Cmac, Mac};
    ...
    let mut mac = <Cmac<Aes128> as Mac>::new_from_slice(key)
        .map_err(|e| VerificationError::internal(format!("CMAC key error: {}", e)))?;
