...
fn duration_to_x509_time(duration: std::time::Duration) -> VerificationResult<Time> {
...
pub fn load_crl_pem(pem_data: &str) -> VerificationResult<CrlInfo> {
    ...
    let (_, der_data) = decode_vec(pem_data.as_bytes())
        .map_err(|e| VerificationError::pem_error(format!("Failed to decode PEM: {}", e)))?;

    load_crl_der(&der_data)
}

/// Load a CRL from DER-encoded data.
pub fn load_crl_der(der_data: &[u8]) -> VerificationResult<CrlInfo> {
    let crl = CertificateList::from_der(der_data)
        .map_err(|e| VerificationError::der_error(format!("Failed to parse DER CRL: {}", e)))?;

    extract_crl_info(&crl)
}

/// Extract information from a parsed CRL.
fn extract_crl_info(crl: &CertificateList) -> VerificationResult<CrlInfo> {
...
pub fn is_certificate_revoked(crl_der: &[u8], serial_hex: &str) -> VerificationResult<bool> {
...
pub fn get_revoked_certificates(crl_der: &[u8]) -> VerificationResult<Vec<RevokedCertInfo>> {
...
pub struct CrlBuilder {
    issuer: super::cert_builder::DistinguishedName,
    validity_days: u32,
...
impl Default for CrlBuilder {
    fn default() -> Self {
        Self {
            issuer: super::cert_builder::DistinguishedName::default(),
            validity_days: 30,
            crl_number: None,
...
impl CrlBuilder {
    ...
    pub fn issuer(mut self, issuer: super::cert_builder::DistinguishedName) -> Self {
        self.issuer = issuer;
        self
    }
    ...
    /// # Returns
    /// DER-encoded CRL
    pub fn build(&self, ca_key_pem: &str) -> VerificationResult<Vec<u8>> {
    ...
    fn build_with_p256(
        ...
        crl.to_der()
            .map_err(|e| VerificationError::internal(format!("Failed to encode CRL: {}", e)))
    }
    ...
    fn build_with_rsa(&self, signing_key: &rsa::RsaPrivateKey) -> VerificationResult<Vec<u8>> {
        use der::asn1::ObjectIdentifier;
    ...
    fn build_issuer_name(&self) -> VerificationResult<Name> {
        use std::str::FromStr;
        ...
        Name::from_str(&name_str)
            .map_err(|e| VerificationError::internal(format!("Failed to parse issuer name: {}", e)))
    }

    fn build_revoked_certs(&self) -> VerificationResult<Vec<RevokedCert>> {
        ...
        for entry in &self.revoked_entries {
            let serial_bytes = hex::decode(&entry.serial_hex)
                .map_err(|e| VerificationError::internal(format!("Invalid serial hex: {}", e)))?;

            let serial = SerialNumber::new(&serial_bytes).map_err(|e| {
                VerificationError::internal(format!("Invalid serial number: {}", e))
            })?;
    ...
    fn build_crl_extensions(&self) -> VerificationResult<Option<x509_cert::ext::Extensions>> {
        ...
        if let Some(crl_num) = self.crl_number {
            ...
            let int_value = der::asn1::Int::new(&num_bytes).map_err(|e| {
                VerificationError::internal(format!("Failed to create CRL number: {}", e))
            })?;

            let value_der = int_value.to_der().map_err(|e| {
                VerificationError::internal(format!("Failed to encode CRL number: {}", e))
            })?;
            ...
            let ext = Extension {
                ...
                extn_value: der::asn1::OctetString::new(value_der).map_err(|e| {
                    VerificationError::internal(format!("Failed to create extension value: {}", e))
                })?,
            };
