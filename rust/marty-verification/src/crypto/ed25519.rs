...
impl Ed25519KeyPair {
    ...
    pub fn from_secret_key(secret: &[u8]) -> VerificationResult<Self> {
        ...
        let bytes: [u8; SECRET_KEY_LENGTH] = secret
            .try_into()
            .map_err(|_| VerificationError::internal("Invalid secret key length".to_string()))?;
...
impl Ed25519VerifyingKey {
    ...
    pub fn from_bytes(bytes: &[u8]) -> VerificationResult<Self> {
        ...
        let bytes_array: [u8; PUBLIC_KEY_LENGTH] = bytes
            .try_into()
            .map_err(|_| VerificationError::internal("Invalid public key length".to_string()))?;

        let key = VerifyingKey::from_bytes(&bytes_array).map_err(|e| {
            VerificationError::internal(format!("Invalid Ed25519 public key: {}", e))
        })?;
    ...
    /// Verify a signature.
    pub fn verify(&self, message: &[u8], signature: &[u8]) -> VerificationResult<()> {
        ...
        let sig_bytes: [u8; SIGNATURE_LENGTH] = signature
            .try_into()
            .map_err(|_| VerificationError::internal("Invalid signature length".to_string()))?;

        let signature = Signature::from_bytes(&sig_bytes);

        self.key.verify(message, &signature).map_err(|e| {
            VerificationError::internal(format!("Ed25519 signature verification failed: {}", e))
        })
    }
...
///
/// 64-byte signature.
pub fn sign(secret_key: &[u8], message: &[u8]) -> VerificationResult<Vec<u8>> {
...
pub fn verify(public_key: &[u8], message: &[u8], signature: &[u8]) -> VerificationResult<()> {
...
pub fn verify_ed25519_spki(
    ...
    let verifying_key = if public_key_der.len() == PUBLIC_KEY_LENGTH {
        ...
        VerifyingKey::from_public_key_der(public_key_der).map_err(|e| {
            VerificationError::internal(format!("Invalid Ed25519 SPKI public key: {}", e))
        })?
    };
...
pub fn parse_private_key_pem(pem: &str) -> VerificationResult<Ed25519KeyPair> {
    ...
    let der = base64::engine::general_purpose::STANDARD
        .decode(&base64_data)
        .map_err(|e| VerificationError::internal(format!("Invalid PEM base64: {}", e)))?;

    parse_private_key_der(&der)
}

/// Parse a DER-encoded Ed25519 private key (PKCS#8 format).
pub fn parse_private_key_der(der: &[u8]) -> VerificationResult<Ed25519KeyPair> {
...
pub fn parse_public_key_pem(pem: &str) -> VerificationResult<Ed25519VerifyingKey> {
    let lines: Vec<&str> = pem
        .lines()
        .filter(|line| !line.starts_with("-----"))
        .collect();
...
pub fn parse_public_key_der(der: &[u8]) -> VerificationResult<Ed25519VerifyingKey> {
    // SPKI for Ed25519:
    // SEQUENCE {
    //   SEQUENCE { OID 1.3.101.112 }
    //   BIT STRING { 32 bytes }
