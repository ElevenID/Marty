...
impl X25519KeyPair {
    ...
    pub fn from_secret_key(secret_bytes: &[u8]) -> VerificationResult<Self> {
    ...
    pub fn agree(&self, peer_public: &[u8]) -> VerificationResult<[u8; 32]> {
...
pub fn x25519_ephemeral_agree(peer_public: &[u8]) -> VerificationResult<([u8; 32], [u8; 32])> {
    if peer_public.len() != 32 {
        return Err(VerificationError::internal(
            "X25519 public key must be 32 bytes".to_string(),
        ));
...
impl P256KeyPair {
    ...
    pub fn from_secret_key(secret_bytes: &[u8]) -> VerificationResult<Self> {
        let secret = P256SecretKey::from_slice(secret_bytes)
            .map_err(|e| VerificationError::internal(format!("Invalid P-256 secret key: {}", e)))?;
        Ok(Self { secret })
    }
    ...
    pub fn agree(&self, peer_public: &[u8]) -> VerificationResult<Vec<u8>> {
        let peer_key = P256PublicKey::from_sec1_bytes(peer_public)
            .map_err(|e| VerificationError::internal(format!("Invalid P-256 public key: {}", e)))?;
...
pub fn p256_agree(secret_key: &[u8], peer_public: &[u8]) -> VerificationResult<Vec<u8>> {
...
impl P384KeyPair {
    ...
    pub fn from_secret_key(secret_bytes: &[u8]) -> VerificationResult<Self> {
        let secret = P384SecretKey::from_slice(secret_bytes)
            .map_err(|e| VerificationError::internal(format!("Invalid P-384 secret key: {}", e)))?;
        Ok(Self { secret })
    }
    ...
    pub fn agree(&self, peer_public: &[u8]) -> VerificationResult<Vec<u8>> {
        let peer_key = P384PublicKey::from_sec1_bytes(peer_public)
            .map_err(|e| VerificationError::internal(format!("Invalid P-384 public key: {}", e)))?;
...
pub fn p384_agree(secret_key: &[u8], peer_public: &[u8]) -> VerificationResult<Vec<u8>> {
...
pub fn ecies_encrypt(
    ...
    use crate::crypto::symmetric::aes_256_gcm_encrypt;
    use rand::RngCore;
...
pub fn ecies_decrypt(
    ...
    use crate::crypto::kdf::hkdf_sha256;
    use crate::crypto::symmetric::aes_256_gcm_decrypt;
