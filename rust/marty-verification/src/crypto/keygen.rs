...
pub fn generate_keypair(key_type: KeyType) -> VerificationResult<GeneratedKey> {
    match key_type {
...
fn generate_ed25519() -> VerificationResult<GeneratedKey> {
...
pub fn generate_ed25519_pem() -> VerificationResult<(String, String)> {
    use base64::Engine;
...
fn generate_x25519() -> VerificationResult<GeneratedKey> {
    use crate::crypto::ecdh::x25519_generate_keypair;
...
fn generate_p256() -> VerificationResult<GeneratedKey> {
    use crate::crypto::ecdh::p256_generate_keypair;
...
pub fn generate_p256_pem() -> VerificationResult<(String, String)> {
    use base64::Engine;
...
fn generate_p384() -> VerificationResult<GeneratedKey> {
    use crate::crypto::ecdh::p384_generate_keypair;
...
fn generate_rsa(bits: usize) -> VerificationResult<GeneratedKey> {
    ...
    let private_der = private_key.to_pkcs1_der().map_err(|e| {
        VerificationError::internal(format!("RSA private key encoding failed: {}", e))
    })?;
    ...
    let public_der = public_key.to_pkcs1_der().map_err(|e| {
        VerificationError::internal(format!("RSA public key encoding failed: {}", e))
    })?;
...
pub fn generate_rsa_pem(bits: usize) -> VerificationResult<(String, String)> {
    ...
    let private_pem = private_key.to_pkcs1_pem(LineEnding::LF).map_err(|e| {
        VerificationError::internal(format!("RSA private key PEM encoding failed: {}", e))
    })?;

    let public_key = private_key.to_public_key();
    let public_pem = public_key.to_pkcs1_pem(LineEnding::LF).map_err(|e| {
        VerificationError::internal(format!("RSA public key PEM encoding failed: {}", e))
    })?;
...
fn generate_symmetric(size: usize, key_type: KeyType) -> VerificationResult<GeneratedKey> {
