...
pub fn generate_rsa_keypair(bits: usize) -> VerificationResult<(Vec<u8>, Vec<u8>)> {
    if bits < 2048 {
    ...
    let private_key = RsaPrivateKey::new(&mut OsRng, bits)
        .map_err(|e| VerificationError::internal(format!("RSA key generation failed: {}", e)))?;
    ...
    let private_der = private_key
        .to_pkcs8_der()
        .map_err(|e| VerificationError::internal(format!("Failed to encode private key: {}", e)))?;
...
///
/// Signature bytes.
pub fn sign_pkcs1_sha256(private_key_der: &[u8], message: &[u8]) -> VerificationResult<Vec<u8>> {
...
pub fn sign_pkcs1_sha384(private_key_der: &[u8], message: &[u8]) -> VerificationResult<Vec<u8>> {
    let private_key = parse_rsa_private_key(private_key_der)?;
...
pub fn sign_pkcs1_sha512(private_key_der: &[u8], message: &[u8]) -> VerificationResult<Vec<u8>> {
    let private_key = parse_rsa_private_key(private_key_der)?;
...
pub fn sign_pss_sha256(private_key_der: &[u8], message: &[u8]) -> VerificationResult<Vec<u8>> {
    let private_key = parse_rsa_private_key(private_key_der)?;
...
pub fn sign_pss_sha384(private_key_der: &[u8], message: &[u8]) -> VerificationResult<Vec<u8>> {
    let private_key = parse_rsa_private_key(private_key_der)?;
...
pub fn sign_pss_sha512(private_key_der: &[u8], message: &[u8]) -> VerificationResult<Vec<u8>> {
    let private_key = parse_rsa_private_key(private_key_der)?;
...
pub fn verify_pss_sha256(
    ...
    let sig = PssSignature::try_from(signature).map_err(|e| {
        VerificationError::invalid_signature("RSA-PSS", format!("Invalid signature format: {}", e))
    })?;
...
fn parse_rsa_public_key(der: &[u8]) -> VerificationResult<RsaPublicKey> {
    ...
    RsaPublicKey::from_public_key_der(der)
        ...
        .map_err(|e| {
            VerificationError::invalid_signature("RSA", format!("Invalid public key: {}", e))
        })
}
...
fn parse_rsa_private_key(der: &[u8]) -> VerificationResult<RsaPrivateKey> {
    ...
    RsaPrivateKey::from_pkcs8_der(der)
        ...
        .map_err(|e| VerificationError::internal(format!("Invalid RSA private key: {}", e)))
}

/// Get the RSA key size in bits.
pub fn get_rsa_key_size(public_key_der: &[u8]) -> VerificationResult<usize> {
    let public_key = parse_rsa_public_key(public_key_der)?;
